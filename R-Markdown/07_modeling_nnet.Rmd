...
I am using k=10 cross validation for all models as well as several combinations of paramters. Training and Testing sets are all approximately the same size 50/50 split. Each model run

Current Results:

| Model    | Data                                                            | Testing | Training |
|:---------|:----------------------------------------------------------------|:--------|:---------|
...
General Model Form: 
nnet(Texting ~ Subject + Age + Gender + Anger + Contempt + 
               Disgust + Fear + Joy + Sad + Surprise + Neutral)
...
mdl.02.test$Predict = predict(fit, mdl.02.test, type = "raw")
y = predict(fit, mdl.02.test, type = "prob")[2]
...
mdl.02.train$Predict = predict(fit, mdl.02.train, type = "raw")
...
row.names(tab) = tab$Subject
tab = tab[,-1]

tab = arrange(tab, desc(Test))
pander(t(round(tab,3)), col.names = subject)
